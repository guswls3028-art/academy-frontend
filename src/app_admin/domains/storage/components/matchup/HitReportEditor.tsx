...
export default function HitReportEditor({ docId, onClose }: Props) {
  ...
  // ── 작성 완료 + 홈페이지 공개 스냅샷 게시 ──
  // submit은 편집 잠금만 담당한다. 공개 성공은 PublicMatchupShowcase 응답으로만 판단한다.

  const submit = useCallback(async () => {
    if (!reportId) return;
    if (isSubmitted) {
      feedback.info("이미 작성 완료된 보고서입니다. 자료실 게시 상태는 보고서 목록에서 확인해 주세요.");
      return;
    }
    ...
    try {
      ...
      const r = await submitHitReport(reportId, { publishToLanding: false });
      setData((prev) => (prev ? { ...prev, report: r } : prev));
      try {
        const card = await publishMatchupShowcase({
          hit_report_id: reportId,
          // 공개 자료 이름은 시험 원문과 엇갈리지 않도록 실제 시험 문서 제목을 우선한다.
          title: documentTitle || reportTitle.trim() || undefined,
        });
        ...
        feedback.successWithAction({
          ...
          description: "제목과 전체 PDF가 공개 링크에 반영됐습니다.",
          action: {
            label: "홈페이지에서 보기",
            onClick: () => {
              window.location.href = `/landing/matchup-board/${card.id}`;
            },
          },
          duration: 8,
        });
        ...
        const detail = (publishError as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
        feedback.warning(
          `보고서 작성은 완료됐지만 홈페이지에는 게시되지 않았습니다. 보고서 목록의 '+ 자료실 게시'로 다시 시도해 주세요.${typeof detail === "string" ? ` (${detail})` : ""}`,
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const unsubmit = useCallback(async () => {
    ...
    const ok = await confirm({
      title: "보고서 작성 다시 열기",
      message: "보고서를 다시 편집할 수 있게 엽니다. 이미 홈페이지에 게시된 스냅샷은 그대로 유지되며 자료실 관리에서 별도로 내릴 수 있습니다.",
      confirmText: "작성 다시 열기",
      cancelText: "그대로 두기",
    });
    if (!ok) return;
    ...
    try {
      ...
      setData((prev) => (prev ? { ...prev, report: r } : prev));
      feedback.success("보고서를 다시 편집할 수 있습니다. 기존 홈페이지 게시물은 유지됩니다.");
    } catch (e) {
      console.error(e);
      feedback.error("작성 다시 열기 실패. 잠시 후 다시 시도해 주세요.");
    } finally {
      ...
      >
        ...
        <div style={headerStyle}>
            ...
            <div style={{ fontSize: 11, color: "var(--color-text-muted)" }}>
              ...
              {isSubmitted && (
                <span
                  ...
                  title="보고서 작성 완료 상태. 다시 편집하려면 우측 '작성 다시 열기' 버튼을 누르세요."
                >
                  작성 완료
                </span>
              )}
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
            ...
            {/* 한 번의 버튼으로 작성 완료 후 실제 공개 스냅샷 게시까지 실행한다. */}
            ...
            {isSubmitted && (
              <Button
                ...
                title="공개 스냅샷은 유지하고 원본 보고서를 다시 편집합니다"
                leftIcon={<Send size={ICON.sm} />}
              >
                {unsubmitting ? "여는 중…" : "작성 다시 열기"}
              </Button>
            )}
            <button
