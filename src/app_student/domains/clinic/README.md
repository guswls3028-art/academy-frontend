...
1. `/student/clinic`의 **예약하기**에서 보강이 필요한 강의·차시·사유를
   ...
   원본 시험·과제명, 단원/범위, 차시를 내부 세로 스크롤로 확인합니다.
...
4. 세션에서 `allow_time_preference`를 허용한 경우에만 학생이 세션 범위 안의
   ...
   예약 확정값이 아니며, 학생·학부모 요청 `memo`와 함께 교직원이 검토합니다.
...
- 대상 요약과 패스카드는 `GET /clinic/idcard/`를 함께 사용합니다. 패스카드는
  `current_result`, 서버 시각, 학원별 3색, 수강 강의와 차시 이력을 표시하며
  2초마다 갱신합니다.
...
- `SUCCESS`는 지정된 3색의 움직이는 배경과 합격 문구를, `FAIL`은 검은색 경고
  화면과 **클리닉 예약 대상자** 문구 및 예약 진입점을 표시합니다.
- 판정값이 없거나 잘못된 응답은 합격으로 추정하지 않고 재시도 오류 화면으로
  실패 폐쇄합니다.
- 판정은 미해결 `ClinicLink`가 소유합니다. 패스카드는 읽기 화면이며 예약·출석·
  미통과 데이터를 직접 변경하지 않습니다.
