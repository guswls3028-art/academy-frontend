...
`/student/idcard` 패스카드로 현재 합격·클리닉 대상 상태를 교직원에게 보여줍니다.
...
- `PASSED`는 미해소 항목이 없는 합격 상태입니다. 활성 예약이 별도로 있어도
  예약 상태와 가까운 예정 일정은 숨기지 않습니다.
- `CLINIC_REQUIRED`는 미해소 항목이 있고 유효 예약·당일 이행 보호가 없는
  상태로, 검은색 경고 화면과 **클리닉 예약 대상자** 및 예약 진입점을 표시합니다.
- `RETURN_ALLOWED`는 미해소 항목은 유지되지만 오늘·미래 `booked` 예약 또는
  오늘 `attended`/완료가 확인된 상태입니다. **클리닉 예약 완료 · 집에 가도 됨**과
  예약 확정·진행 상태, 대표 일정의 날짜·시작 시각·장소를 표시합니다.
  당일 이행 보호는 다음 KST 로컬 날짜에 끝나며, 미해소 항목이 남으면 다시
  `CLINIC_REQUIRED`가 됩니다. `cancelled`/`rejected`/`no_show`는 보호하지 않습니다.
- `pending`은 **승인 대기** 일정은 표시하지만 귀가 보호가 아니며, 승인 전까지
  `CLINIC_REQUIRED`와 귀가 불가 안내를 유지합니다.
