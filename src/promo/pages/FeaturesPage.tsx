    ...
    <>
          ...
          {GROUPS.map((g) => (
            <div key={g.category}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-bold text-gray-900">{g.category}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {g.items.map((item) => (
                  <div key={item.title} className="p-5 rounded-xl border border-gray-100 bg-white hover:shadow-md transition-shadow">
      ...
      <section className="py-12 text-center">
        <div className="flex flex-wrap justify-center gap-4">
          <Link to="/promo/ai-grading" className="px-6 py-3 bg-blue-50 text-blue-600 font-semibold rounded-lg hover:bg-blue-100 transition-colors text-sm">
            AI 자동채점 상세 →
          </Link>
          <Link to="/promo/video-platform" className="px-6 py-3 bg-violet-50 text-violet-600 font-semibold rounded-lg hover:bg-violet-100 transition-colors text-sm">
            동영상 플레이어 상세 →
          </Link>
        </div>
      </section>
