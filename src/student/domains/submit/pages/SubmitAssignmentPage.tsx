...
import { IconImage, IconVideo, IconDownload } from "@/student/shared/ui/icons/Icons";
...
function FileIcon({ file }: { file: InventoryFile }) {
  ...
  if (ct.startsWith("video/") || file.icon === "video") return <IconVideo style={s} />;
  return <IconImage style={s} />;
}
...
export default function SubmitAssignmentPage() {
  ...
  // 최근 과제 제출 (description에 "과제" 포함 or icon=video/image, 최근 5개)
  const recentAssignments = (inventory?.files ?? [])
    .filter((f) => f.description?.includes("과제") || ((f.icon === "video" || f.icon === "image") && f.folderId === null))
  ...
  const uploadMut = useMutation({
    mutationFn: async () => {
      ...
      const icon = selectedFile.type.startsWith("video/") ? "video" : "image";
  ...
  return (
    <StudentPageShell
      title="과제 제출"
      description="동영상 또는 사진을 올리면 인벤토리에 저장됩니다."
