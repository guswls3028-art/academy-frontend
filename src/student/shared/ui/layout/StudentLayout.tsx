...
/**
 * ✅ StudentLayout (LOCK v2)
 *
 * 책임:
 * - 학생앱 전역 레이아웃 "디자인" SSOT
 * - 상단바 + 중앙(공지/일정) + 페이지 컨텐츠 + 하단탭바
 *
 * 원칙:
 * - 도메인 로직/데이터 판단 ❌
 * - 라우팅/권한 판단 ❌
...
export default function StudentLayout() {
  return (
    ...
    >
      ...
      >
        <div className="stu-page" style={{ padding: "var(--stu-space-10) var(--stu-space-8)" }}>
          {/* ✅ 상단 공지 + 일정 (중앙 상단 영역) */}
          <StudentHomeStrip />

          {/* ✅ Page content */}
          <div style={{ marginTop: "var(--stu-space-10)" }}>
            <Outlet />
          </div>
        </div>
      </main>
