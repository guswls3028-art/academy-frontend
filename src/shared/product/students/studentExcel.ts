// PATH: src/shared/product/students/studentExcel.ts
...
export async function downloadStudentExcelTemplate(
  ...
  const guide = [
    ...
    ["  신규 학생·학부모 계정 비밀번호는 업로드할 때 직접 입력하거나 안전한 임시 비밀번호 생성을 선택합니다."],
  ...
  // ── 셀 스타일 (xlsx 기본 지원 범위) ──
  sheet.mergeCells(1, 1, 1, headers.length); // 제목
  sheet.mergeCells(3, 1, 3, headers.length); // "작성 안내"
  // 안내문 행 병합 (각 행)
  for (let r = 3; r <= 7; r++) {
    sheet.mergeCells(r + 1, 1, r + 1, headers.length);
  }
  sheet.getRow(1).font = { bold: true, size: 14 };
  sheet.getRow(10).font = { bold: true };

  await downloadWorkbook(workbook, "학생_일괄등록_양식.xlsx");
}
...
export async function parseStudentExcel(file: File): Promise<ParseStudentExcelResult> {
  try {
        ...
        if (headerRowIndex < 0) {
          throw new Error("양식이 맞지 않습니다. 이름·전화번호 컬럼이 있는 행을 찾을 수 없습니다.");
        }
...
export async function downloadStudentsExcel(
  ...
  await downloadArrayWorksheet({
    ...
    sheetName: "학생목록",
    rows: data,
    columnWidths: headers.map((_, i) => (i === 0 ? 12 : i === 9 ? 18 : 14)),
  });
}
