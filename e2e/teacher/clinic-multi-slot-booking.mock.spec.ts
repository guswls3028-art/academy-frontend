...
test("선생님이 학생 여러 명을 17시부터 19시까지 두 시간대에 원자적으로 추가한다", async ({ page }) => {
  ...
  await addStudentTrigger.click();
  const keyboardSheet = page.getByRole("dialog", { name: "학생 추가" });
  const closeButton = keyboardSheet.getByRole("button", { name: "닫기" });
  await closeButton.focus();
  await page.keyboard.press("Shift+Tab");
  expect(await keyboardSheet.evaluate((element) => element.contains(document.activeElement))).toBe(true);
...
test("선생님이 모바일과 데스크톱에서 예약 승인부터 하원·완료 취소까지 같은 lifecycle을 운영한다", async ({ page }, testInfo) => {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (statusMatch && request.method() === "PATCH") {
      ...
      if (payload.status === "booked" && rejectFirstApproval) {
        rejectFirstApproval = false;
        return json({ detail: "승인 상태가 변경됐습니다. 다시 확인해 주세요." }, 409);
      }
  ...
  await expect(page.getByText("승인 상태가 변경됐습니다. 다시 확인해 주세요.")).toBeVisible();
  await expect(approveRow).toContainText("승인 대기");
  await approveRow.getByRole("button", { name: "예약 승인" }).click();
  ...
  expect(await page.getByTestId("teacher-clinic-participant-910")
    .getByRole("button", { name: "등원" })
    .evaluate((element) => element.getBoundingClientRect().height)).toBeGreaterThanOrEqual(44);
  await page.screenshot({ path: testInfo.outputPath("teacher-lifecycle-390.png"), fullPage: true });

  await page.setViewportSize({ width: 1366, height: 900 });
  await expect(page.getByTestId("teacher-clinic-participant-910")).toBeVisible();
  expect(await page.locator("body").evaluate((element) => element.scrollWidth <= element.clientWidth)).toBe(true);
  await page.screenshot({ path: testInfo.outputPath("teacher-lifecycle-1366.png"), fullPage: true });
});
