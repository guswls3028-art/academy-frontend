...
test("미식별 제출 인박스 — 가드 + 액션 분기 + confirm 다이얼로그", async ({ page }) => {
  ...
  if (bannerVisible) {
    ...
    // ── 3) 폐기 클릭 → confirm 다이얼로그 노출 → 취소 ──
    const firstDiscard = page.getByRole("button", { name: "폐기" }).first();
    await firstDiscard.click();
    await expect(page.getByText("이 답안지를 폐기할까요?").first()).toBeVisible({ timeout: 5_000 });
    await page.getByRole("button", { name: "취소" }).click();
    await expect(page.getByText("이 답안지를 폐기할까요?")).toHaveCount(0, { timeout: 3_000 });
  } else {
