...
test.describe("DNB 영상/스태프/저장소/도구/개발자 검증", () => {
  ...
  test("01. 영상 탐색기 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/videos", "dnb-01-videos");
  });

  // ──────────────── 저장소 ────────────────

  test("02. 저장소 > 내 저장소 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/storage", "dnb-02-storage");
  });

  test("03. 저장소 > 학생 인벤토리 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/storage/students", "dnb-03-storage-students");
  });
  ...
  test("04. 스태프 목록 렌더링", async ({ page }) => {
    ...
    const staffLink = page.locator('a[href*="/workspace/staff"], nav >> text=스태프').first();
    if (await staffLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      await staffLink.click();
      await page.waitForLoadState("networkidle", { timeout: 5_000 }).catch(() => {});
    } else {
      await gotoAndSettle(page, `${DNB_BASE}/workspace/staff`, { settleMs: 1500 });
    }
  ...
  test("05. 스태프 출근부 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/staff/attendance", "dnb-05-staff-attendance");
  });

  test("06. 스태프 경비 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/staff/expenses", "dnb-06-staff-expenses");
  });

  test("07. 스태프 월 마감 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/staff/month-lock", "dnb-07-staff-month-lock");
  });

  test("08. 스태프 급여 스냅샷 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/staff/payroll-snapshot", "dnb-08-staff-payroll-snapshot");
  });

  test("09. 스태프 리포트 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/staff/reports", "dnb-09-staff-reports");
  });

  test("10. 스태프 설정 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/staff/settings", "dnb-10-staff-settings");
  });

  // ──────────────── 도구 ────────────────

  test("11. 스톱워치 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/tools/stopwatch", "dnb-11-stopwatch");
  });

  test("12. 가이드 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/guide", "dnb-12-guide");
  });

  // ──────────────── 개발자 ────────────────

  test("13. 버그 리포트 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/developer/bug", "dnb-13-bug-report");
  });

  test("14. 피드백 렌더링", async ({ page }) => {
    await visitAndAssertRendered(page, "/workspace/developer/feedback", "dnb-14-feedback");
  });
});
