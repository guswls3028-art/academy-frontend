...
test.describe("성적 탭 UX 개선 검증", () => {
  ...
  /**
   * 4. 안전 잠금·자동 저장·키보드 단축키
   */
  test("2. 수정 중 자동 저장 후 다시 잠그는 흐름 확인", async ({ page }) => {
    const ok = await navigateToScoresTab(page);
    if (!ok) { test.skip(); return; }

    // 잠금 상태에서는 상세 조회·발송은 가능하고 점수 셀은 수정되지 않아야 한다.
    ...
    console.log("[알림톡] 수정 중에는 발송 차단 확인됨");
    ...
    await expect(page.locator(".ds-scores-cell-editable")).toHaveCount(0);
    await expect(scoreSendButton).toBeEnabled();
    console.log("[입력 잠금] 저장 완료 후 발송·조회 상태 복귀 확인됨");
  });
