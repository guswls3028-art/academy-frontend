 ...
 *   - submit → poll → R2 fetch → PPTX extract → PIL 픽셀 평균 검증.
 *
...
function extractPptx(pptxPath: string, extractDir: string): void {
  if (process.platform === "win32") {
    execFileSync(
      ...
      [
        ...
        "Bypass",
        "-Command",
        "& { param([string]$PptxPath, [string]$ExtractDir) $ErrorActionPreference = 'Stop'; Expand-Archive -LiteralPath $PptxPath -DestinationPath $ExtractDir -Force }",
        pptxPath,
        extractDir,
      ],
      { stdio: "pipe" },
  ...
  execFileSync("unzip", ["-q", pptxPath, "-d", extractDir], { stdio: "pipe" });
}

function runPython(script: string, env: NodeJS.ProcessEnv): string {
...
test.describe("PPT 생성기 — 이미지 일괄 흑백반전 (운영 결과 검증)", () => {
  test("이미지 5장 + 흑백반전 토글 → 5장 모두 반전된 PPTX 생성", async ({ request }) => {
    test.setTimeout(540_000); // worker cold-start + 5장 변환 여유 포함
    ...
    let slide_count = 0;
    let lastStatus = "";
    let lastProgressBody: unknown = null;
    for (let i = 0; i < 210; i++) {
    ...
    expect(download_url, `PPT job did not finish: job_id=${jobId} last=${JSON.stringify(lastProgressBody)}`).toBeTruthy();
    ...
    extractPptx(pptxPath, extractDir);
