...
async function installApi(
  ...
  await page.route("**/api/v1/**", async (route: Route) => {
    ...
    if (path === "/clinic/participants/bulk-create/" && method === "POST") {
      ...
      const targetSession = sessionRows.find((session) => session.id === sessionId);
      const created = targetIds.map((targetId, index) => {
        ...
        const row = {
          ...
          session_date: targetSession?.date ?? saturday,
          session_start_time: targetSession?.start_time ?? "17:00:00",
          session_location: targetSession?.location ?? "2층 보강실",
          session_title: targetSession?.title ?? "토요일 5시 클리닉",
          status: "booked",
          source: "manual",
      ...
      if (targetSession) {
        targetSession.participant_count += created.length;
        targetSession.booked_count += created.length;
      }
...
test("자유지정 클리닉 생성과 동시에 배정할 학생의 실제 이용 시간을 저장하고 다시 읽는다", async ({ page }) => {
  const operationsState: OperationsState = {
    participants: [],
    targets: [{
      enrollment_id: 1501,
      student_id: 601,
      student_name: "자유배정 학생",
      session_title: "수학 미통과",
      clinic_reason: "exam",
      reason: "score",
      clinic_link_id: 9501,
      session_id: 3501,
  ...
  const scheduleState: ScheduleState = { createPayloads: [], updatePayloads: [] };

  await seed(page);
  await installApi(page, undefined, operationsState, scheduleState);
  ...
  const dialog = page.getByRole("dialog", { name: "클리닉 만들기" });
  await dialog.getByRole("button", { name: /자유지정 클리닉/ }).click();
  await dialog.getByPlaceholder("예: 수학 보충").fill("자유 배정 클리닉");
  await dialog.getByPlaceholder("장소 / 룸").fill("자유 학습실");
  const timePopover = page.getByRole("dialog", { name: "시간 선택" });
  await dialog.getByRole("button", { name: "시작 시간 선택", exact: true }).click();
  await timePopover.getByLabel("분 단위 직접 입력").fill("15:00");
  await timePopover.getByRole("button", { name: "적용", exact: true }).click();
  ...
  await dialog.getByRole("button", { name: "대상자 추가" }).click();
  const targetDialog = page.getByRole("dialog", { name: "대상자 선택" });
  await targetDialog.getByRole("checkbox", { name: "자유배정 학생 선택" }).check();
  await targetDialog.getByRole("button", { name: "선택 확정 (1명)" }).click();

  const actualTime = dialog.getByRole("region", { name: "실제 예약 시간" });
  await expect(actualTime).toBeVisible();
  const createButton = dialog.getByRole("button", { name: "1명 배정하고 클리닉 만들기" });
  await expect(createButton).toBeDisabled();
  await actualTime.getByRole("button", { name: "16:00 시작, 잔여 10자리" }).click();
  await actualTime.getByRole("button", { name: "19:00 종료, 총 3시간" }).click();
  await expect(actualTime).toContainText("1명에게 같은 16:00–19:00 구간을 적용합니다.");
  await expect(createButton).toBeEnabled();
  expect(await dialog.evaluate((element) => element.scrollWidth <= element.clientWidth)).toBe(true);
  expect(await actualTime.evaluate((element) => element.scrollWidth <= element.clientWidth)).toBe(true);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  await actualTime.scrollIntoViewIfNeeded();
  await page.screenshot({ path: "test-results/admin-clinic-create-range-assignment-390.png", fullPage: false });
  await createButton.click();

  const confirmation = page.getByRole("alertdialog", { name: "클리닉 일정 최종 확인" });
  await expect(confirmation).toContainText("자유배정 학생 · 1명 · 16:00–19:00");
  await confirmation.getByRole("button", { name: "확인하고 만들기" }).click();

  await expect.poll(() => scheduleState.createPayloads[0]).toMatchObject({
    title: "자유 배정 클리닉",
    start_time: "15:00:00",
    duration_minutes: 420,
    booking_mode: "time_range",
  });
  await expect.poll(() => operationsState.participantBulkPayloads?.[0]).toEqual({
    session_ids: [799],
    enrollment_ids: [1501],
  ...
  await page.goto(`${BASE}/workspace/clinic/operations?scope=day&date=${saturday}&session=799`, {
    waitUntil: "domcontentloaded",
  });
  await expect(page.getByText("자유배정 학생", { exact: true })).toBeVisible();
  await expect(page.getByText("예약 16:00–19:00", { exact: true })).toBeVisible();
...
test("시간 범위 수업은 누적 예약이 동시 정원보다 많아도 열린 구간을 마감하지 않는다", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/workspace\/clinic\/operations/);
  await page.setViewportSize({ width: 390, height: 844 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
});
...
test("월간 달력에서 원하는 날짜를 고르면 그날 일정만 명확히 표시한다", async ({ page }) => {
  ...
  await expect(page.getByRole("grid", { name: /클리닉 월간 달력/ })).toHaveCount(0);
  await page.getByRole("button", { name: "달력으로 이동" }).click();
  ...
  await expect(page.getByRole("dialog", { name: "날짜 이동" })).toHaveCount(0);
  await expect(page).toHaveURL(new RegExp(`date=${saturday}`));
  const selectedDay = page.getByRole("grid", { name: /클리닉 예약 일정/ })
    .getByRole("gridcell", { name: new RegExp(`^${saturdayLabel}`) });
  await expect(selectedDay.getByRole("article")).toHaveCount(3);
  ...
  await expect.poll(async () => Math.min(...await page.getByRole("grid", { name: /클리닉 월간 달력/ })
    .getByRole("gridcell")
...
test("달력 팝업에서 월 이동 후 날짜를 고르면 생성 날짜가 바뀐다", async ({ page }) => {
  ...
  await expect(nextMonthFirstDay).toHaveAttribute("aria-selected", "false");
  await expect(page).not.toHaveURL(/date=/);
  await nextMonthFirstDay.click();
  await expect(page.getByRole("dialog", { name: "날짜 이동" })).toHaveCount(0);
  await expect(page.getByRole("grid", { name: /클리닉 예약 일정/ })
...
test("달력 팝업에서 다음 달 날짜를 고르면 선택한 날짜 일정으로 이어진다", async ({ page }) => {
  ...
  await expect(page.getByRole("dialog", { name: "날짜 이동" })).toHaveCount(0);
  await expect(page).toHaveURL(new RegExp(`date=${nextMonthDate}`));
  const selectedDay = page.getByRole("grid", { name: /클리닉 예약 일정/ })
...
test("오늘 요약에서 일정과 통합 운영 화면으로 이동하고 전환 상태를 새로고침 후 유지한다", async ({ page }) => {
  await seed(page);
  await installApi(page);
  await page.setViewportSize({ width: 1366, height: 850 });
  await gotoAndSettle(page, `${BASE}/workspace/clinic/home`, { timeout: 45_000 });

  await expect(page.getByRole("heading", { name: "오늘 한눈에" })).toBeVisible();
  await expect(page.getByRole("button", { name: /오늘 운영/ })).toBeVisible();
  await page.getByRole("button", { name: /일정 배정/ }).click();
  await expect(page.getByRole("grid", { name: /클리닉 예약 일정/ })).toBeVisible();
  await page.goto(`${BASE}/workspace/clinic/home`, { waitUntil: "domcontentloaded" });
  await page.getByRole("button", { name: /미통과 처리/ }).click();
  const switcher = page.getByRole("navigation", { name: "클리닉 운영 화면" });
  await expect(switcher.getByRole("button", { name: "미통과 처리 · 전체 기간" }))
    .toHaveAttribute("aria-current", "page");
  await page.reload({ waitUntil: "domcontentloaded" });
  await expect(switcher.getByRole("button", { name: "미통과 처리 · 전체 기간" }))
    .toHaveAttribute("aria-current", "page");
  await switcher.getByRole("button", { name: "예약·출석" }).click();
  await expect(page.getByRole("heading", { name: "클리닉 진행" })).toBeVisible();
  await page.setViewportSize({ width: 390, height: 844 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
});
...
test("관리자가 패스카드 3색을 확인하고 학생 화면에 적용한다", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "학생 화면 예시" })).toBeVisible();
  await page.getByRole("button", { name: "예약 완료" }).click();
  await expect(page.getByText("예약완료", { exact: true })).toBeVisible();
  await page.getByRole("button", { name: "승인 대기" }).click();
  await expect(page.getByText("클리닉 대상 · 승인 대기" )).toBeVisible();
  await page.getByRole("button", { name: "예약 필요" }).click();
  await expect(page.getByText("과락 요소 있음")).toBeVisible();
  await page.getByRole("button", { name: "합격", exact: true }).click();
  await expect(page.getByText("합격자", { exact: true })).toBeVisible();
