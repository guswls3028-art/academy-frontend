...
test.describe("커뮤니티 QnA 작업대", () => {
  ...
  test("390px 자료/답변 전환에서 작성 내용을 보존하고 가로 넘침이 없다", async ({ page }) => {
    ...
    await page.screenshot({ path: "test-results/community-qna-attachments-390.png", fullPage: true });
  ...
  test("비이미지 다운로드 실패는 파일을 유지하고 다시 누를 수 있게 복구한다", async ({ page }) => {
    ...
    await expect(pdfDownload).toBeVisible();
    await page.screenshot({ path: "test-results/community-qna-file-recovery-390.png", fullPage: true });
  });
