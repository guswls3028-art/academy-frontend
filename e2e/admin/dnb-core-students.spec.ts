...
test.describe("DNB 코어 + 학생 관리 E2E", () => {
  ...
  test("11. 개발자 > 패치노트 렌더링", async ({ page }) => {
    ...
    await expect(page.getByTestId("pn-modal")).toBeVisible();
    await expect(page.getByText("발송 수단을 알림톡으로 명확히 고정", { exact: false })).toBeVisible();
    await page.getByRole("button", { name: "릴리스 상세 닫기" }).click();
    await expect(page.getByTestId("pn-modal")).toBeHidden();
