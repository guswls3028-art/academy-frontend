...
test.describe("매치업 시각 리뷰", () => {
  ...
  test("V1. 좌측 트리 — 글로벌 카운트 + 검색", async ({ page }) => {
    ...
    await waitForAnyVisible(page, "[data-testid='matchup-doc-search'], [data-testid='matchup-doc-row']");
  ...
  test("V2. 검색 hint — 필터 결과 N건", async ({ page }) => {
    await openMatchup(page);
    await waitForAnyVisible(page, "[data-testid='matchup-doc-search']");
  ...
  test("V3. 행 hover — 휴지통/··· 노출", async ({ page }) => {
    ...
    // hover 후
    await row.hover();
    await row.locator("[data-testid='matchup-doc-row-more']").waitFor({ state: "visible", timeout: 2000 }).catch(() => {});
    await page.screenshot({
  ...
  test("V4. ··· 메뉴 — 4 항목 (이름/intent/카테고리/삭제)", async ({ page }) => {
    ...
    await waitForAnyVisible(page, "[role='menuitem']");
  ...
  test("V5. 카테고리 변경 sub-popover", async ({ page }) => {
    ...
    await catBtn.waitFor({ state: "visible", timeout: 3000 });
    await catBtn.click();
    await waitForAnyVisible(page, "[role='menu'], [role='dialog'], [class*='popover']");
  ...
  test("V6. 완료 doc 디테일 — 액션 버튼 + 카운트", async ({ page }) => {
    ...
    await row.click();
    await waitForAnyVisible(page, "[data-testid='matchup-problem-card'], [data-testid='document-guidance-banner']", 8000);
  ...
  test("V7. 처리중 doc — PDF disabled + 단계 체크리스트", async ({ page }) => {
    ...
    await row.click();
    await waitForAnyVisible(page, "text=/문제 분할|텍스트 추출|AI 분석|처리 중|업로드/", 8000);
