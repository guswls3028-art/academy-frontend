...
test("매치업 skeleton INSERT 정식 검증 — 신규 업로드 → 즉시 카드 → 점진 채움", async ({ page }) => {
  ...
  const ts = Date.now();
  const tag = `[E2E-${ts}]`;
  const fixturePath = path.resolve(__dirname_, "../fixtures/test-matchup.pdf");
  ...
  let visibleCardsAwaitingTerminalLogged = false;
  ...
  for (let i = 0; i < 180; i++) {  // 최대 6분
    ...
    if (skeletonObserved && partialBadges === 0 && allCards > 0 && apiStatus === "done") {
    ...
    if (
      ...
      partialBadges === 0 &&
      allCards > 0 &&
      !["done", "failed"].includes(apiStatus) &&
      !visibleCardsAwaitingTerminalLogged
    ) {
      visibleCardsAwaitingTerminalLogged = true;
      log(`카드 표시 후 API terminal 대기 — status=${apiStatus}, cards=${allCards}`);
    }
    ...
    if (skeletonObserved && ["done", "failed"].includes(apiStatus)) {
      log(`API terminal 상태 도달 — status=${apiStatus}, problem_count=${apiProblemCount}`);
      break;
    }
    // eslint-disable-next-line no-restricted-syntax
    await page.waitForTimeout(2000);
  ...
  if (!skeletonObserved) {
    ...
    expect(
      terminalWithoutProblems,
      `skeleton 없이 종료된 경우에는 terminal 상태가 UI에 보여야 함. status=${apiStatus}, row="${rowText.slice(0, 120)}", progressLabelObserved=${progressLabelObserved}`,
    ).toBe(true);
  } else if (apiProblemCount > 0) {
    expect(finalCards).toBeGreaterThan(0);
  }
  expect(apiStatus, `신규 업로드가 처리중으로 멈추면 안 됨. doc=${docId}, problem_count=${apiProblemCount}`).toBe("done");
  expect(apiProblemCount, `신규 업로드 완료 후 문항 수가 0이면 안 됨. doc=${docId}`).toBeGreaterThan(0);
  expect(finalCards, "완료 후 문제 카드가 보여야 함").toBeGreaterThan(0);
  expect(finalPartialBadges, "완료 후 처리 중 뱃지가 남으면 안 됨").toBe(0);
  ...
  // assertion: skeleton card가 보였거나, 최소한 완료된 문제 카드가 사용자에게 표시되어야 한다.
  expect(skeletonObserved || finalCards > 0).toBe(true);
});
