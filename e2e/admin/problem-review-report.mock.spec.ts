...
async function installApp(page: Page) {
  ...
  await page.route("**/mock-files/**", (route) => route.fulfill({
    status: 200,
    headers: { "Content-Disposition": "attachment; filename=problem-review.pdf" },
  ...
  await page.route("**/api/v1/**", async (route: Route) => {
    ...
    if (path === `/tools/problem-review/reports/${REPORT_ID}/exports/export-job-1/`) return json({
      job_id: "export-job-1",
      status: "DONE",
      result: { download_url: `${BASE}/mock-files/problem-review.pdf`, filename: "문제리뷰.pdf", size_bytes: 24, output_format: "pdf" },
    });
...
test("문제 리뷰를 검수 저장하고 PDF로 내려받으며 390px에서도 넘치지 않는다", async ({ page }, testInfo) => {
