...
test.describe("tchul.com 외부 관전자 시각 검수", () => {
  test("desktop 1920 — root 진입 + 전체 fullPage 캡처", async ({ browser }) => {
    ...
    await gotoAndSettle(page, `${PROD_BASE}/landing`, { timeout: 20_000 });
    await expect(page.locator("nav").first()).toBeVisible({ timeout: 10_000 });
    await expect(page.locator("text=/적중률/").first()).toBeAttached({ timeout: 10_000 });
  ...
  test("login 화면 모바일 + 데스크탑", async ({ browser }) => {
    for (const viewport of [{ name: "desktop-1920", w: 1920, h: 1080 }, { name: "mobile-390", w: 390, h: 844 }]) {
      ...
      const page = await ctx.newPage();
      await gotoAndSettle(page, `${PROD_BASE}/login`, { timeout: 20_000 });
      await expect(page.getByRole("button", { name: /로그인/ }).first()).toBeVisible({ timeout: 10_000 });
