...
test.describe("조교 로그인 출근 선택", () => {
  for (const width of [1366, 390]) {
    test(`${width}px에서 지연 로그인과 계정 안내를 마친 뒤 성적 화면을 준비한다`, async ({ page }) => {
      ...
      const scoresRequested = page.waitForRequest((request) => new URL(request.url()).pathname === "/api/v1/results/admin/sessions/41/scores/", { timeout: 30_000 });
      const scoresBody = page.waitForResponse((response) => new URL(response.url()).pathname === "/api/v1/results/admin/sessions/41/scores/", { timeout: 30_000 })
        .then(async (response) => {
