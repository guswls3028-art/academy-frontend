...
async function stubLoginBootstrap(page: Page) {
...
async function openRecovery(page: Page, mode: "username" | "password") {
...
test.describe("계정 복구 모달 UI 검증", () => {
  ...
  test("아이디 찾기 모달이 열리고 학생/학부모 대상 전환이 동작한다", async ({ page }) => {
    ...
    await expect(dialog.getByText("등록된 정보가 확인되면 입력하신 휴대폰 번호로 안내를 보내드립니다.")).toBeVisible();
  ...
  test("아이디 찾기 성공 시 서버 안내 문구를 표시한다", async ({ page }) => {
    const message = "입력한 정보가 등록되어 있다면 해당 번호로 아이디 안내가 발송됩니다.";
  ...
  test("비밀번호 찾기 성공 시 임시 비밀번호 발송 안내를 표시한다", async ({ page }) => {
    const message = "입력한 정보가 등록되어 있다면 해당 번호로 임시 비밀번호가 발송됩니다.";
  ...
  test("발송 중에는 제출 버튼이 비활성화되어 중복 요청을 막는다", async ({ page }) => {
    ...
    await page.route("**/api/v1/auth/account-recovery/dispatch/**", async (route) => {
      ...
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ message: "입력한 정보가 등록되어 있다면 해당 번호로 임시 비밀번호가 발송됩니다." }),
      });
    });
    const dialog = await openRecovery(page, "password");
    ...
    await page.keyboard.press("Enter");
    await expect(dialog.getByRole("status")).toHaveText("입력한 정보가 등록되어 있다면 해당 번호로 임시 비밀번호가 발송됩니다.");
