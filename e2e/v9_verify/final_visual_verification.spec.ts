...
async function waitForSettledUi(page: Page, timeout = 15_000) {
  await page.waitForLoadState("networkidle", { timeout }).catch(() => {});
  await expect(page.locator("body")).toBeVisible({ timeout });
}

async function clickAndWaitForVisible(trigger: Locator, expected: Locator, page: Page) {
  await trigger.click({ timeout: 5000 });
  await waitForSettledUi(page);
  await expect(expected.first()).toBeVisible({ timeout: 10_000 });
}

async function fillSearchAndWait(searchBox: Locator, value: string, page: Page, expected: Locator) {
  await searchBox.click({ clickCount: 3 });
  await page.keyboard.press("Backspace");
  await searchBox.fill(value);
  await expect(searchBox).toHaveValue(value, { timeout: 3000 });
  await waitForSettledUi(page);
  await expected.first().waitFor({ state: "visible", timeout: 7000 }).catch(() => {});
}
...
test.describe("최종 종합 시각 검증", () => {
  ...
  test("02 학원장 적중 보고서 모음", async ({ page }) => {
    ...
    if (await reportListBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await clickAndWaitForVisible(reportListBtn, page.locator("text=/중대부고|숙명여고/"), page);
  ...
  test("03 보고서 #14 중대부고 진입 — 학원장 3 + 자동 33 entries", async ({ page }) => {
    ...
    const reportListBtn = page.getByText(/학원 적중 보고서 모음/, { exact: false }).first();
    if (await reportListBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await clickAndWaitForVisible(reportListBtn, page.locator('text=/중대부고.*기출 통과|2026 중대부고 1학기/'), page);
    }

    // #14 보고서 클릭 (중대부고)
    const report14 = page.locator('text=/중대부고.*기출 통과|2026 중대부고 1학기/').first();
    if (await report14.isVisible({ timeout: 3000 }).catch(() => false)) {
      await clickAndWaitForVisible(report14, page.locator('[data-testid="matchup-problem-card"]'), page);
      ...
      if (await firstProblem.isVisible({ timeout: 3000 }).catch(() => false)) {
        await clickAndWaitForVisible(firstProblem, firstProblem, page);
      ...
      if (await thirdProblem.isVisible({ timeout: 3000 }).catch(() => false)) {
        await clickAndWaitForVisible(thirdProblem, thirdProblem, page);
      ...
      if (await fifthProblem.isVisible({ timeout: 3000 }).catch(() => false)) {
        await clickAndWaitForVisible(fifthProblem, fifthProblem, page);
  ...
  test("04 보고서 #25 숙명여고 — indexable 복구 검증", async ({ page }) => {
    const reportListBtn = page.getByText(/학원 적중 보고서 모음/, { exact: false }).first();
    if (await reportListBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await clickAndWaitForVisible(reportListBtn, page.locator('text=/숙명여고.*1학기 중간/'), page);
    }

    const report25 = page.locator('text=/숙명여고.*1학기 중간/').first();
    if (await report25.isVisible({ timeout: 3000 }).catch(() => false)) {
      await clickAndWaitForVisible(report25, page.locator('[data-testid="matchup-problem-card"]'), page);
  ...
  test("05 자동분리 sample — 메인자료 자료 시각", async ({ page }) => {
    ...
    await fillSearchAndWait(searchBox, "빅뱅과 원소의 생성 중대부고", page, firstDoc);
    ...
    if (await firstDoc.isVisible({ timeout: 3000 }).catch(() => false)) {
      await clickAndWaitForVisible(firstDoc, firstDoc, page);
    ...
    await fillSearchAndWait(searchBox, "2026 중대부고 1학기 중간", page, doc148);
    if (await doc148.isVisible({ timeout: 3000 }).catch(() => false)) {
      await clickAndWaitForVisible(doc148, doc148, page);
