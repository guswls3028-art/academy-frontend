...
async function scrollAndClick(page: Page, buttonText: RegExp, timeout = 5000): Promise<boolean> {
  ...
  try {
    ...
    // pointer intercept 문제 — dispatchEvent로 클릭 시뮬레이션
    try {
...
test("선생님 체험: 로그인 → 학생등록 → 수강등록 → 시험생성 → 점수입력 → 저장 → 확인", async ({ page }) => {
  ...
  if (await addExamBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (await newExam.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      // 기본값 "시험 1" 그대로 사용 — 바로 "일괄 생성" 클릭
      await page.waitForTimeout(500);
      const created = await scrollAndClick(page, /일괄 생성/);
      console.log(`시험 일괄 생성: ${created ? "✓" : "⚠"}`);
      await page.waitForTimeout(4000);
      await snap(page, "exam-created");

      // 모달이 닫혔는지 확인
      const modalStillOpen = await page.locator('text=일괄 생성').first().isVisible({ timeout: 1000 }).catch(() => false);
  ...
  // 시험 생성 후 — 모달 닫기 대기 → 성적 탭이 자동 갱신되는지 확인
  await page.waitForTimeout(3000);
  await snap(page, "scores-after-exam-create");

  // 성적 탭이 현재 화면인지 확인 (URL에 /scores 포함)
  const currentUrl = page.url();
  console.log(`현재 URL: ${currentUrl}`);

  // 성적 탭을 다시 클릭해서 데이터 새로고침
  ...
  await snap(page, "scores-refreshed");
