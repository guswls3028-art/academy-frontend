...
- 생성된 문자·문단·스타일 서식과 편집 가능한 한글 수식을 유지한 채 삽입합니다.
- 검수본에 선생님 개인 글꼴이 있으면 서버의 크기·SHA-256을 확인한 뒤 현재 Windows 사용자에게 설치할지 묻습니다. 거절하면 대체 글꼴로 계속 열 수 있습니다.
- 개인 글꼴은 HWPX나 배포 ZIP에 내장하지 않으며, 현재 교사가 만든 일회용 연결에서만 내려받습니다.
- 기존 문서를 저장·닫기·종료하지 않습니다.
- 다운로드 URL이나 인증 토큰을 custom URI에 직접 넣지 않으며, handoff는 `api.hakwonplus.com` HTTPS만 허용합니다.
- 다운로드는 768MB로 제한되고, HWPX는 정확히 한 개만 허용하며 50MB를 넘으면 중단합니다. 개인 글꼴은 파일당 32MB, 문서당 4개로 제한합니다.
