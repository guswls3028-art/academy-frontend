...
그 뒤 `Inspect`에서 tenant/user/R2/runtime 잔여가 모두 0이어야만 정리가 완료된다.
...
`GET /api/v1/community/posts/<post_id>/`의 200 `post_type=qna` 응답에서만
현재 browser context에 서명 URL을 등록한다. URL은 Setup tenant와 해당 게시글의
...
간주하지 않는다. 단일 자녀만 자동 확정하고, 다자녀 학부모는 테넌트·학부모별로 저장된
유효한 직접 선택이 없으면 학생 범위 API를 호출하지 않은 채 선택 화면에서 멈춘다. 각
쓰기·조회 전에 화면의 자녀 선택 탭으로 의도한 학생을 명시 선택한다.
...
재생 시작 응답은 동일 playback 경로에서 query가 없거나 선택적인 양의 정수
`enrollment`만 있는 `POST`를 bootstrap으로 분류한다. 주기적인 접근 확인은
`GET ?access_check=1`과 선택적인 양의 정수
...
본문을 읽지 않고 취소한다. 인증 단계의 exact 401 기대를 다른 status로 넓히지 않는다.
