...
거부를 모두 통과해야 한다. 학생 fixture 정리는 영구삭제 응답의 `deleted` 수와
`storage_cleanup.pending=0`, `failed=0`을 직접 확인한 뒤에만 성공으로 처리한다.
...
실제 frontend IAM role/document 적용과 20 PASS/0 SKIP/양쪽 cleanup0 증거가 모두 있어야
이 전환 HOLD를 해제할 수 있다.
