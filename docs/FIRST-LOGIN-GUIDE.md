...
2. owner·student·parent가 `must_change_password=true`이면 허용 화면 위에 변경
   권장 화면을 표시한다. 사용자는 위험 고지를 확인하고 나중에 변경해 계속
   이용할 수 있다. admin·teacher·staff에는 이 권장 상태를 적용하지 않는다.
