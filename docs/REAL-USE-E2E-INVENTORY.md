...
| `.github/workflows/e2e.yml` | PR 빠른 gate. 수동 실행은 전 메뉴 감사를 제외한 core를 2개 shard로 나눈 뒤 desktop admin/dev, student mobile, teacher mobile 전 메뉴 감사를 역할별 독립 job으로 실행 | core 2개와 전 메뉴 3개 job은 운영 E2E tenant의 인증 throttle/상호 간섭을 피하려고 각각 `max-parallel: 1`로 직렬 실행. 각 job은 120분 상한이며 전체 workflow 성공은 core 2개와 감사 3개가 모두 성공해야 함 |
