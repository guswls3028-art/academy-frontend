...
| 문항별 직접 채점 | `e2e/admin/manual-exam-grading.mock.spec.ts` | 성적표 시험명에서 채점표 진입, O/X/0과 사용자 지정 키의 입력 후 자동 이동, 자동채점 선택형의 O/X 조회, 문항 없는 시험의 답안 등록·빠른 시작, 인라인 배점 합계, 기기 저장, 미리보기와 확정 POST 분리, 확정 후 서버 재조회 상태를 1100px 관리자 화면에서 검증 | 로컬 route-mock. 저장·tenant/role·선택형 답안 보존·혼합형 OMR 보존·배점 검증은 백엔드 `apps/support/results/tests/test_manual_exam_grading.py`가 검증 |
