...
언제 오는지, 곧 누구를 준비해야 하는지, 내일은 몇 명인지 한눈에 파악하게 한다.
...
- 상단: 오늘 날짜, 다음 등원 학생과 남은 시간
- 요약: 1시간 내, 오늘, 내일, 시간 미정, 필요한 경우 예정 시간 지남
- 시간 레일: 시간 순 학생, 보강/클리닉 출처, 강의·장소·메모, 처리 여부
- 오른쪽: 내일 학생을 최대 6명까지 미리 보여 주고 초과 인원을 표시
...
예정 시간 지남은 오류 톤, 1시간 내는 경고 톤, 시간 미정은 정보 톤이다. 교사용
모바일 알림 센터에서도 항목 타입을 안전하게 표시하고 PC 대시보드 현황으로
연결한다.

## 빈 상태와 실패 경계

- 오늘 예정이 없으면 입력 위치를 안내하는 완료형 빈 상태를 표시한다.
- 통합 조회가 실패하면 0명으로 보이지 않게 오류와 `다시 불러오기`를 표시한다.
- 내일 항목이 없으면 별도 빈 문구를 표시한다.
...
핵심 회귀는 보강 전용 열, 저장 payload와 재조회, 두 출처 시간순 표시, 내일 준비,
세 종류 알림, 상세 이동 경계, 1366/390px overflow와 콘솔 오류 0건이다.
