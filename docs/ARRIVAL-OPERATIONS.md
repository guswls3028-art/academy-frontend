...
`오늘 처리할 일`의 미답변 질문·제출 채점·클리닉 승인 대기는 우상단 알림과
같은 운영 집계 캐시를 사용한다. 특히 클리닉 승인 대기는 승인 완료 뒤 등원
목록에 들어오는 `booked` 상태에서 추정하지 않고 `pending` 예약 수를 직접
표시한다. 알림과 대시보드가 동시에 열려도 등원 현황 요청은 같은 20초 캐시를
공유해 30초 갱신마다 중복 호출하지 않는다. 부분 집계 실패는 0건으로 위장하지
않고 해당 카드에 `확인 필요`를 표시한다.
...
준비, 실제 `pending` 클리닉 승인 수, 대시보드·헤더의 등원 요청 1회 공유, 빈 상태
직접 동선, 네 종류 알림, 상세 이동 경계, 1366/390px overflow와 콘솔 오류 0건이다.
