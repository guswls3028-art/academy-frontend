...
출석 탭을 처음 여는 계정의 명단은 이름 가나다순으로 표시한다. 교직원이 이름,
상태, 학부모 전화번호, 학생 전화번호 머리글로 정렬을 바꾸면 같은 계정과
브라우저의 다른 차시 출석 화면에서도 마지막 정렬을 유지한다. 저장 키는 계정
ID별로 분리해 공용 PC에서 다른 계정의 선택이 섞이지 않으며, 브라우저 저장소를
사용할 수 없거나 저장값이 유효하지 않으면 이름 가나다순으로 복구한다.

정렬 설정은 화면 표시만 바꾸며 출결 데이터나 서버 상태를 수정하지 않는다.
검색·상태 필터 결과가 바뀌어도 현재 정렬을 적용하고, 기존 머리글의 오름차순,
내림차순, 정렬 없음 순환 동작을 유지한다.
