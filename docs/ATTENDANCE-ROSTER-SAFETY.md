...
실제 `ABSENT`는 해당 차시의 시험·과제 todo에서 제외되지만 `ONLINE`은 영상
학습 요청 상태이므로 todo 대상이다. 전체 현장 출석으로 `ABSENT`를 `PRESENT`로
바꾸거나 개별 상태를 `ONLINE/PRESENT`로 고치면 서버가 같은 트랜잭션에서 활성
정규 시험·과제 대상을 멱등하게 복구한다. 되돌리기로 다시 `ABSENT`가 되면 성적
명단 행은 유지하고 미수행 평가 칸과 현재 클리닉 대상만 제외한다. 기존 점수·제출·
수동 처리·완료 이력은 어느 전환에서도 삭제하지 않는다.
...
충돌 시 전체 거부, `ONLINE/PRESENT` todo 복구와 실제 `ABSENT` 제외,
1366/1100/390px overflow와 콘솔 오류 0건이다.
