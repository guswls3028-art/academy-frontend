...
| 직원 비밀번호 설정 | 직원관리에서 한 명 선택 → 비밀번호 변경 | `POST /staffs/{id}/change-password/` + `password` | 대상 기존 토큰 폐기, 설정값으로 즉시 로그인 가능, 변경 강제·권장 없음 |
...
- 직원 프로필 저장과 비밀번호 설정은 서로 다른 버튼과 요청으로 분리한다.
