...
| 관리자·선생님 비밀번호 초기화 | 학생 상세에서 `비밀번호 초기화` | staff 학생/학부모 초기화 API + 직접 입력한 `temp_password` | 선택 계정만 입력값으로 초기화하고 승인된 알림톡 발송 |
...
- 가입 결과는 `POST /students/registration_requests/`의 기존 응답으로 구분한다.
  `200 + StudentDetail`이면 가입 완료·즉시 로그인 가능 안내와 `로그인하기` 버튼을,
  `201 + status=pending` 가입신청이면 기존 승인대기 안내와 `확인` 버튼을 표시한다.
  둘 다 버튼을 누르면 원래 테넌트 로그인 화면으로 돌아가며 자동 로그인이나 토큰
  발급을 하지 않는다. 자동승인 여부를 프런트 설정으로 추정하거나 학생 응답을
  승인대기 가입신청으로 변환하지 않는다. 정책과 계정 생성은
  [백엔드 학생 생성 계약](../../backend/docs/domain/student-creation.md)이 소유한다.
- 정상 응답의 식별자가 없거나 지원하지 않는 결과, 비-2xx 응답은 완료로 표시하지
  않는다. 기존 오류·중복가입 안내를 유지하고 입력을 보존한다. 결과 확인이 불가능한
  경우 재신청 전에 선생님에게 확인하도록 안내하며 자동 재시도는 하지 않는다.
  완료 안내와 버튼은 데스크톱·390px에서 잘리지 않아야 한다. 기존
  `signup-tenant-policy.mock.spec.ts`는 200/201 안내, 오류 후 수동 재시도, 테넌트와
  비인증 요청, 로그인 복귀·다시 열기 초기화, DOM 경계와 스크린샷을 검증한다.
  흰 가입 카드의 제목·완료 문구는 카드의 짙은 본문색을 상속하고, 제출·완료 버튼은
  기존 테넌트 그라데이션 위의 어두운 대비층으로 흰 글자를 유지한다. 이 규칙은
  가입 카드 안에만 적용하며 다른 로그인·복구 모달이나 배치·간격은 바꾸지 않는다.
  네 가지 완료 렌더는 제목·문구·버튼의 최소 4.5:1 대비도 함께 검사한다.
  이 로컬 모의 응답 검증은 실제 계정 저장·로그인·승인대기 persistence의 증거가
  아니며, 해당 실사용 검증은 격리된 개발 환경에서 별도로 수행한다.
...
학생 관리의 삭제 학생 일괄 복원은 별도 복원 모달을 사용한다. 정상 학부모 계정은
기존 비밀번호와 알림 상태를 그대로 보존한다. 누락·사용불가 학부모 계정이 있으면
서버가 `parent_account_password_required`를 반환하며, 모달에서 선생님이 4자 이상
초기 비밀번호를 명시 입력해 그 학생만 다시 시도한다. 새 자격 증명을 만든 경우에만
학부모 알림톡을 예약하고, 예약 실패 시 해당 학생 복원과 계정 생성을 함께 취소한다.
...
- 학생·학부모 단일/일괄 초기화는 4자 이상 임시 비밀번호를 직원이 직접 입력해야
  한다. 빈값 자동 생성이나 전화번호 파생값은 없으며, 선택한 대상에 입력값을
  적용하고 승인된 알림톡을 발송한다.
...
- 비밀번호 초기화 화면은 4자 이상 입력 전 실행할 수 없고 대상별 파괴적 변경을 명시하며, 아이디 안내 성공 문구를 재사용하지 않는다.
...
- `위험을 이해했고 나중에` 선택은 현재 tenant·user·인증 generation에만 귀속한다.
  같은 인증 세션의 경로 이동과 새로고침에서는 다시 업무를 막지 않지만, 로그아웃 뒤
  새 로그인, 다른 사용자 또는 다른 tenant에서는 변경 권장을 다시 표시한다.
