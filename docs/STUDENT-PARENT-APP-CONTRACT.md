...
- 학부모는 명시적으로 선택한 연결 자녀의 온라인 시험 답안과 과제 사진·영상을
  대리 제출할 수 있다. 제출·목록·재접속 요청은 모두 같은 `X-Student-Id`를 사용하고,
  자녀를 바꾸면 이전 자녀의 쿼리 캐시와 시험 초안을 가져오지 않는다. 성적표 제출,
  자녀 보관함 파일 변경, 자녀 프로필 변경, 질문·상담 작성은 계속 읽기 전용이다.
  학부모 자신의 비밀번호 변경은 허용하며,
