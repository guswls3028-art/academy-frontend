...
- 입력은 debounce 저장하고 페이지 숨김·브라우저 이탈·학생 앱 내부 화면 이동 전에
  flush한다. 저장 중·저장 완료·복구·저장 실패를 폼 안에서 보여 주며, 저장소 실패가
  질문 또는 상담 제출 자체를 막지는 않는다.
- 같은 계정의 다른 탭에서 더 최신 초안이 생겨도 현재 내용을 자동 덮어쓰지 않는다.
  사용자가 새 초안을 불러오거나 현재 내용을 유지하도록 명시적으로 선택한다.
- 첨부파일 이름·bytes·미리보기는 초안에 보관하지 않는다. 첨부가 있었던 초안을
  복구하면 본문을 유지하고 파일은 다시 선택하라고 안내한다. 서버 등록이 성공한
  뒤에만 해당 초안을 제거하며, API 실패 시 입력과 초안을 그대로 유지한다.
