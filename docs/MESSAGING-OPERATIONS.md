...
교사용 `/workspace/mobile/message-log`도 동일한 처리 단계와 읽기 전용 최종 상태
확인을 제공한다. 교사·조교의 본문 및 공급자 식별자 제한은 그대로 유지한다.
...
교사용 목록도 `발송 준비 중`, `접수 확인 중`, `접수 완료`, `재시도 대기`,
`발송 실패`, `결과 확인 필요`를 구분한다. `success` boolean으로 최종 전달을
판정하지 않으며 상태가 없거나 알 수 없으면 `상태 확인 필요`를 표시한다.
목록의 **다시 불러오기**는 현재 테넌트의 목록 GET만 수행한다. 조회 실패는
빈 이력으로 바꾸지 않고 오류와 **다시 시도**를 제공한다.

공급자 접수 증거가 있는 교사용 기록에는 **최종 상태 확인**을 제공한다. 버튼을
누를 때만 기존 `GET /messaging/log/{id}/?verify_provider=true`를 호출하며,
조회 중 중복 클릭 및 자동 재시도를 하지 않는다. 서버의 확인 결과는 `공급사 접수`,
`최종 전달 확인`, `최종 전달 실패`, `최종 상태 확인 불가`로 별도 표시한다.
실패 시 접수 기록은 유지하고 **다시 확인**으로 복구한다. 상세 응답의 본문·정확한
공급자 ID를 이 카드에 추가 노출하지 않는다. 새로고침 후에는 목록의 처리 상태를
다시 읽으며, 이전 최종 확인을 영구 저장된 전달 완료로 추정하지 않는다.
이 경로는 발송·재발송이 아니며 요청 ID별 발송 추적 계약을 새로 만들지 않는다.
...
overflow를 고정한다.
교사용 전용 회귀 `e2e/teacher/message-log-lifecycle.mock.spec.ts`는 닫힌 로컬
API proxy에서 교사·조교 × 1366/390px의 처리 단계, 최종 확인 GET, 실패·재시도,
reload, 개인정보 제한과 메시징 mutation 0을 검사한다. mock 통과는 실제 공급자
전달이나 운영 발송 성공의 증거가 아니며, 실발송 없이 허용된 개발 검증을 별도로
수행해야 한다.
