...
직접 발송과 미리보기 확인 발송이 접수되면 화면은 이를 `발송 완료`로 부르지
않는다. 즉시 발송은 한 사용자 동작에 같은 `request_id`를 부여해 학생·학부모
요청과 worker 로그를 묶고, 성공 알림의 **발송 내역 확인**으로 그 exact 요청만
조회한다. URL의 `origin_id` 필터는 새로고침 뒤에도 유지된다. 상세 화면의
**최종 상태 확인**을 실행해야 공급사 접수와 최종 전달 완료를 구분할 수 있다.
