...
시험이 하나면 해당 시험의 업로드 창을 바로 열고, 여러 시험이면 먼저 시험을
고른다. 업로드 창에는 등록 대상 시험명을 크게 표시해 잘못된 시험으로 올리는
실수를 줄인다. 대상 선택·업로드 상태는 기존 시험별 API 경계를 따른다.
...
빈 성적표의 자동 입력 준비가 진행 중이면 `OMR 스캔 등록`은 준비 중 안내를 표시하고,
준비가 끝난 뒤 현재 화면의 입력 잠금을 마무리한 다음 업로드 모달을 연다. 첫 렌더와
비동기 잠금 획득 사이에도 이 순서를 지킨다. 새로고침 전 문서나 다른 탭이 점유한
첫 성적 셀은 자동 선택·점유하지 않고, 실제 입력 가능한 셀만 선택한다. 모두 잠겼으면
선택을 비워 두어 OMR 등록을 계속할 수 있다. 다른 문서의 잠금을 넘겨받거나 해제하지
않으며, 문서별 client id와 서버의 셀 충돌 검증은 유지한다.
...
- `e2e/admin/score-entry-autosave.spec.ts`는 이전 문서의 셀 잠금을 유지한 reload,
  지연된 자동 입력 준비, 잠긴 첫 셀의 자동 점유 금지와 OMR batch 생성→파일 접수를 검증한다.
