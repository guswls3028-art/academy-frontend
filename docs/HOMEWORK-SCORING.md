...
관리자·선생님은 **강의 → 차시**에서 과제를 만들고, 과제별 만점과 차시
공통 합격 정책을 설정한 뒤 **성적** 탭에서 학생 점수를 입력한다.

```text
과제 A 만점 ─┐
과제 B 만점 ─┼─ 차시 공통 합격 정책 ─ 성적표 분모·합격·클리닉 판정
과제 C 만점 ─┘
```

- **과제 만점**은 선택한 과제 하나의 설정이다. 문제 수가 다르면 각각 다른
  만점을 지정한다.
- **과제 정책**은 같은 차시의 모든 과제에 적용된다. 만점이 다르면 퍼센트
  기준을 권장한다. 원점수 기준은 가장 낮은 과제 만점을 넘을 수 없다.
...
- 과제 생성 모달은 제목과 `max_score`를 한 요청으로 저장한다. 만점이 1
  미만이거나 원점수 커트라인보다 낮으면 아무 과제도 만들기 전에 오류를
  표시한다.
- 다른 차시 과제를 복사할 때 원본 과제의 만점을 보존한다. 100점으로
  재설정하지 않는다.
