...
혼합 시험에서 OMR 객관식 결과가 먼저 확정된 뒤 서술형을 입력해도 두 값은 같은 1차
응시다. 서버는 서술형 확정 시 canonical 합계와 1차 projection을 함께 갱신하고 최초
OMR 출처·제출 식별자는 보존한다. 이후 재시험이 생기면 결과 목록, 석차·동점, 통계,
분석·내보내기, 합불·클리닉, 교사·학생·학부모 화면은 OMR+서술형 확정 합계를 1차
점수로 사용하며 재시험 점수를 섞지 않는다. 점수 저장 자체는 메시지를 발송하지 않는다.
