...
- Ymath 자료 업로드는 한 화면에서 세 형태를 받는다. 통합 PDF/HWP/HWPX 하나,
  ...
  통합 HWP/HWPX는 모든 번호의 본문 문제와 미주 원본 그림을 각각 재현하고
  `한글 본문·미주 분리`로 표시한다. 일부만 있으면 부분 결과를 성공으로 간주하지
  않고 정답 표시 없는 문제 PDF를 안내한다. 과거 `hwp_endnote` 후보의 8~98%
  문제 영역 조절은 진행 중 작업을 위한 호환 동작이며 신규 분리 방식이 아니다.
- 한 번에 최대 100문항이다.
