...
| 문제 이미지에 정답 필기가 섞임 | 업로드한 주 파일, 선택 해설 HWP/HWPX, `한글 본문·미주 분리` 표시 | 통합 HWP/HWPX는 미주 번호로 본문 문제와 미주 해설을 별도 재현해야 한다. 새 후보에 표시가 없거나 정답 필기가 보이면 확정하지 않고 정답 표시 없는 문제 PDF와 해설 HWP/HWPX를 함께 올린다. |
...
- Ymath 자료 업로드는 한 화면에서 세 형태를 받는다. 통합 PDF/HWP/HWPX 하나,
  ...
  못하면 부분 결과를 성공으로 간주하지 않고 정답 표시 없는 문제 PDF를 안내한다.
