...
5. 오답노트 출력·회차 범위·PDF/HWPX 선택은 `WrongNotePanel.tsx`,
   ...
   형식과 관계없는 단일 문제+해설, 문제 파일, 선택 해설 원본 업로드는
...
| 문제 이미지에 정답 필기가 섞임 | 업로드한 주 파일, 선택 해설 원본, `한글 본문·미주 분리` 표시 | 통합 HWP/HWPX는 미주 번호로 본문 문제와 미주 해설을 별도 재현해야 한다. 새 후보에 표시가 없거나 정답 필기가 보이면 확정하지 않고 답 표시가 없는 별도 문제 원본과 해설 원본을 각각 보관해 직접 검수한다. |
...
- Ymath 자료 업로드는 한 화면에서 형식과 관계없이 통합 원본 하나, 문제 원본
  하나, 또는 문제와 교사 해설 원본 한 쌍을 받는다. 실행·스크립트·브라우저 실행
