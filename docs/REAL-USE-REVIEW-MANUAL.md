...
디자인·편의성·동선 변경 요청은 제안된 해법부터 구현하지 않는다. 영향을 받는
현재 화면과 실제 역할별 진입 → 작업 → 저장 → 다음 화면을 먼저 확인하고,
사용자의 불편 제보, 관찰된 결함, 아직 확인하지 못한 추정을 구분한다. 실행
화면을 볼 수 없다면 해당 라우트·컴포넌트·데이터 상태와 기존 증거로 진단하고
미확인 부분을 표시한다.

관련성이 큰 경우만 골라 발견성·정보 위계·문구·조작 수·피드백·접근성·기존
디자인 일관성을 평가한다. 빈/긴 데이터, 지연·실패·재시도, 중복 입력, 뒤로가기·
새로고침·미저장 이탈, 역할·권한·tenant, 키보드·터치·390px를 가능성과 피해에
따라 우선순위화한다. 작은 수정에 전 메뉴 감사나 모든 조합 테스트를 요구하지
않는다. 구현 전에는 **현재 불편과 근거 → 우선순위가 높은 예외 → 선택한 개선안과
대안의 차이**를 짧게 남긴다. 사용자의 제안과 다른 결론이면 관찰 근거를 설명한다.
구현 후에는 기존 합격 기준에 따라 성공, 실패·복구, 저장 후 소비 화면을 검증한다.
...
범위에 해당하는 역할·목표 동선을 정확한 SHA와 환경에서 실제 렌더로 확인하고,
자동 DOM 검사·route mock 통과·스크린샷 생성과 사람이 본 판정을 따로 기록한다.
보지 못한 화면·상태는 미검증으로 남긴다.
...
| 칸·밀도 | 셀 크기·간격·정렬·가독성이 작업에 맞고 스크롤·overlay 뒤에도 다음 행동이 가능 | 답답한 칸, 과밀, 잘린 값, 가려진 CTA |
| 표시 데이터 | 값의 의미와 현재 사용 가능 여부를 역할이 판단할 수 있음 | 빈값·0·미산출·오래된 값·권한 밖 값을 혼동하거나 실행에 쓸 수 없음 |
...
15. 빈 성적표라도 복구 draft가 있거나 다른 화면이 수정 중이면 자동 입력하지 않는지 확인한다.
    ...
    - 공식 `omr-review-realuse.spec.ts`의 기존 단일 test는 관리자 문항별 채점과 별개로 disposable tenant에 ASSISTANT를 생성하고 `/core/me/`의 `tenantRole=staff`를 확인한다. 실제 로그인 후 workspace 도착 → 최초 계정 안내 → **출근하지 않고 로그인** 선택을 완료한 뒤 정확한 성적 URL과 해당 시험을 포함한 점수 API의 200 응답·표시 옵션 준비를 확인한다. 점수 응답 본문은 도착 즉시 읽어 보관하고 로그인 reload 후 해당 시험 메타를 검증하여 이전 문서의 응답 객체에 의존하지 않는다. 두 화면에서 주관식 합계 셀 입력·저장 → 열린 다른 화면의 빈 점유 → 명시적 재개 → 원래 점수 복원·reload를 PC/390px에서 검증하고 학생·학부모 합계가 유지되는지 읽는다. 직원 client별 lease는 직원 token으로 정리하고 Staff를 삭제하며, 남은 합성 User/token은 공식 tenant cleanup0가 확인한다. 테스트 코드 추가만으로 실행 성공을 주장하지 않고 해당 artifact의 공식 canary 결과를 별도 확인한다.
...
17. 실제 인쇄·필기·스캔한 혼합형 OMR을 등록해 객관식만 판독된 시점에는
    **서술형 점수 입력 필요**가 보이고, 교직원에게만 객관 점수가 보이는지 확인한다.
    이때 학생·학부모 결과, 석차·평균, 클리닉·오답 후속, 알림톡, CSV·성적표 PDF에는
    부분점수가 없어야 한다. 서로 다른 교직원 브라우저에서 서술형을 입력하고 저장한 뒤
    새로고침해 최종점수·석차·후속 상태가 정확히 한 번 생기는지 확인한다. 펜·연필,
    기울임, 저해상도, 수정테이프, 학생 식별 성공·불명확 샘플을 포함하고 불명확한 답안은
    자동 확정하지 않는다.
18. 성적 탭의 **개인 성적표**에서 학생 전환, 다중 선택, 요약 1쪽/상세
    페이지 전환, 단일·다중 PDF 다운로드가 저장된 점수와 일치하는지
    확인한다.
19. **성적 도구 → 성적표 출력**에서 로고가 비율을 유지하고 긴 평가명이
    잘리지 않으며, 학생 수가 많을 때 A4 가로 여러 쪽으로 분리되는지 확인한다.
20. **성적 도구 → 클리닉 대상**에서 서버의 미해소 `ClinicLink` 대상만 시험·과제
    구분에 맞게 보이고, 현장 출석·지각·보강만 전체 출석과 명단에 포함되는지
    확인한다. 같은 대상 링크가 있어도 `영상(ONLINE)` 학생은 제외하고, 출결 조회
    실패 시 명단을 추정하지 않고 다시 시도를 제공해야 한다.
    시험 대상에서 한 학생만 제거한 뒤 새로고침하여, 해당 학생의 칸은
    `미배정`이고 성적표·출력 명단·학생 요약에서 클리닉 대상이 아닌지,
    계속 배정된 학생의 대상 상태는 유지되는지 함께 확인한다.
21. 점수가 있는 비만점 시험의 오답 확인과 점수 없는 종이 과제 검사를 각각
...
- 직원 UI 로그인 후 계정 안내를 확인하고 필수 출근 선택창에서 `출근하지 않고 로그인`을 명시적으로 선택해 창이 닫힌 뒤 성적표로 이동한다. 빈 성적표는 자동 입력 모드의 준비 완료를 기다린다. 91점 저장 후에는 읽기 모드의 기존 점수를 확인하고 `수정`으로 다시 입력한다. 로딩 중 입력창의 순간 노출 여부로 두 경로를 선택하지 않으며, 각 저장 응답·reload·최종 학생/학부모92점 검증을 유지한다.
...
- 원본이 유효해도 학생이 응시·제출 미배정이면 그 원본의 과거 실패
  `ClinicLink`로 대상이 되지 않고, 제거 이력만 남으며 알림은 0건이다.
...
- [ ] 정확한 SHA·환경·역할별 목표 동선에서 실제 렌더와 데이터 사용 가능성을 판정하고, 자동검사 결과와 미관찰 범위를 구분했다.
