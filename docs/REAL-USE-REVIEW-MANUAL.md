...
14. 서로 분리된 두 브라우저로 같은 차시 성적표를 열어 과제 셀 협업을 확인한다.
    ...
    - 두 화면에서 시험 합산·객관식·주관식·문항 및 과제의 서로 다른 셀을 동시에 입력할 수 있고, 상대가 선택한 정확한 셀만 `이름 + 입력 중`으로 막히는지 확인한다. 셀 선택 전환 중 일시적으로 같은 셀 점유 응답을 받은 뒤 다른 셀의 협업 상태 갱신이 성공하면, 오래된 충돌 안내가 해제되고 `저장하고 잠금`을 정상 완료할 수 있는지 확인한다. 충돌 뒤에도 입력 중 로컬 초안이 유지되고 새로고침 복구가 가능한지 확인한다.
