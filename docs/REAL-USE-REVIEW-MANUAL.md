...
14. 서로 분리된 두 브라우저로 같은 차시 성적표를 열어 과제 셀 협업을 확인한다.
    - 서로 다른 셀을 각각 저장하면 양쪽 화면의 깨끗한 셀이 새로고침 없이 같은 값으로 수렴하는지 확인한다.
    - 한쪽이 같은 셀의 로컬 값을 입력해 둔 상태에서 다른 쪽이 먼저 저장해도 로컬 입력이 원격 조회로 덮이지 않는지 확인한다.
    - 한쪽이 같은 셀을 선택만 한 상태에서 다른 쪽이 먼저 저장하면 선택 화면의 옛값이 유지되는지 확인한다. 그 뒤 옛값을 보고 입력·저장했을 때도 선택 시점의 version으로 conflict가 나고, 첫 저장 서버값과 내 입력이 모두 보존되어야 한다. 숫자 입력 셀과 완료형 선택 셀을 각각 확인한다.
    - 같은 셀의 두 번째 저장은 conflict가 되고 **최신 서버값 · 내 입력**과 **내 점수 다시 적용**이 보이며, conflict 시점의 서버값이 첫 저장값 그대로인지 확인한다.
    - 학생 상세 드로어에서 1차 과제 점수 편집을 시작한 뒤 다른 화면이 먼저 저장해도, 드로어의 옛 입력과 편집 시작 version이 함께 유지되어 conflict가 나는지 확인한다. 드로어에서도 서버값·내 입력 비교와 명시 재적용 뒤 저장·새로고침을 확인한다.
    - 재적용 뒤 양쪽이 같은 값으로 수렴하고 두 브라우저를 새로고침한 뒤에도 저장값이 유지되는지 확인한다.
    - 상대가 선택한 셀에는 편집자 이름이 표시되고, 표시되지 않은 다른 과제 셀은 계속 입력 가능한지 확인한다.
    - 두 화면에서 시험 합산·객관식·주관식·문항 및 과제의 서로 다른 셀을 동시에 입력할 수 있고, 상대가 선택한 정확한 셀만 `이름 + 입력 중`으로 막히는지 확인한다. 셀 선택 전환 중 먼저 보낸 정상 응답을 지연시킨 채 최근 셀의 점유 충돌 응답을 먼저 받아도 늦은 과거 응답이 충돌 상태를 해제하지 않는지 확인한다. 이후 다른 셀의 협업 상태 갱신이 성공하면 오래된 충돌 안내가 해제되고 `저장하고 잠금`을 정상 완료할 수 있어야 한다. 충돌 뒤에도 입력 중 로컬 초안이 유지되고 새로고침 복구가 가능한지 확인한다.
...
16. 성적표의 시험명을 눌러 OMR 자동채점 시험은 **OMR 검토**, 직접 채점
    ...
    Windows·Linux `Ctrl+S`/Mac `⌘S` 확인·확정과 브라우저 저장 차단,
...
19. **성적 도구 → 클리닉 대상**에서 서버의 미해소 `ClinicLink` 대상만 시험·과제
    구분에 맞게 보이고, 현장 출석·지각·보강만 전체 출석과 명단에 포함되는지
    확인한다. 같은 대상 링크가 있어도 `영상(ONLINE)` 학생은 제외하고, 출결 조회
    실패 시 명단을 추정하지 않고 다시 시도를 제공해야 한다.
20. 점수가 있는 비만점 시험의 오답 확인과 점수 없는 종이 과제 검사를 각각
    완료→재조회→미완료로 되돌린다. 실패 응답에서는 서버 검증 사유가 보이고
    UI가 성공 상태로 오인되지 않는지 확인한다. 저장 성공 직후 성적표 재조회가
    일시 실패해도 완료 표시를 유지하고, 같은 점수·답안의 재동기화 뒤에는 완료가
    유지되지만 실제 점수 또는 답안 변경 뒤에는 검수 대기로 돌아가는지 확인한다.
    완료 사유가 2자 미만일 때 **통과 확정/완료 확정**을 누르거나 키보드로 실행하면
    저장 요청 없이 사유 입력으로 포커스가 이동하고 즉시 안내가 보여야 한다. 2자
    이상 입력한 뒤에만 기존 완료 저장을 실행하며, 미응시·무점수 시험·만점·점수
    미저장 상태의 잠금은 그대로 유지한다.
...
- 별도 외부 프로그램 붙여넣기용 클리닉 도구와 `/workspace/tools/clinic` 기능은
  존재하지 않으며, 클리닉 출력은 성적 도구의 저장 데이터 경로만 사용한다.
...
9. 출석 처리한다.
10. 학생 클리닉 대상 요약과 성적 화면에서 요건 해소가 반영되는지 본다.
