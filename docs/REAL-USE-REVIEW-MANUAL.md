...
| 일반 토스트 | 표시 중에도 뒤의 입력·버튼을 즉시 조작할 수 있음 | hover로 닫힘이 멈추거나 아래 조작을 가로챔 |
...
버튼을 포함한 `successWithAction` 알림은 의도적으로 상호작용할 수 있어야 한다. 그 외
성공·실패·정보·경고 토스트는 입력을 막지 않고 hover와 무관하게 정해진 시간 뒤 사라진다.
...
14. 서로 분리된 두 브라우저로 같은 차시 성적표를 열어 과제 셀 협업을 확인한다.
    ...
    - 셀 선택 전환 중 일시적으로 같은 셀 점유 응답을 받은 뒤 다른 셀의 협업 상태 갱신이 성공하면, 오래된 충돌 안내가 해제되고 `저장하고 잠금`을 정상 완료할 수 있는지 확인한다.
