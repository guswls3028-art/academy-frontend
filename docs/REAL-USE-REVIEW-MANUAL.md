...
2. 지원 파일을 업로드한다.
3. 업로드 완료 후 처리 상태가 PENDING/PROCESSING/READY로 전이되는지 본다.
4. READY 전 학생에게 노출되지 않는지 확인한다.
5. READY 후 학생 앱 `영상`에서 강의->차시->영상을 찾는다.
6. 영상을 재생, 일시정지, seek, 이어보기한다.
7. 댓글/좋아요를 남긴다.
8. 관리자 영상 상세에서 시청률/댓글/학생별 로그가 반영되는지 본다.
9. 모바일 네트워크 지연 상태에서 플레이어가 빈 화면으로 멈추지 않는지 본다.
