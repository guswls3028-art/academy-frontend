# tchul 공식 홈페이지와 매치업 자료실

## 목적과 사용자

`tchul.com/landing`은 박철T의 수업과 성과 자료를 처음 접하는 학생·학부모를
위한 공식 홈페이지다. 단일 홍보 문구를 길게 소비시키는 일반적인 랜딩 페이지가
아니라, 대기업 제품 사이트처럼 핵심 정보와 근거 자료로 빠르게 이동하는 구조를
사용한다.

- 외부 방문자는 첫 화면에서 강사·수업의 정체성, 매치업 자료실, 상담 동선을
  구분해 찾을 수 있어야 한다.
- 원장/관리자는 컴퓨터에 저장해 직접 수정한 PDF를 일반 게시글처럼 올리고,
  게시물 전용 링크를 문자나 카카오톡 홍보에 사용할 수 있어야 한다.
- 기존 자동 생성 매치업 보고서는 보조 호환 경로로 유지한다. 홈페이지의 기본
  게시 흐름은 직접 PDF 업로드다.

## 정보 구조와 라우트

| 경로 | 역할 |
|---|---|
| `/landing` | 짧은 공식 홈. 첫 화면, 빠른 메뉴, 최신 매치업 3건, 시험 분석 최신 2건, 수업 기준, 강사 정보, 상담 |
| `/landing/about`, `/landing/guide` | 강사·수업·관리 방식 소개와 수강 전 안내 |
| `/landing/matchup-board` | 공개 매치업 자료 아카이브 |
| `/landing/matchup-board/:id` | 문자·카카오톡에 공유할 수 있는 단일 자료 페이지 |
| `/landing/analysis` | 선생님 검수 완료 시험 분석 공개 목록 |
| `/landing/analysis/:id` | 총평·출제 축·단원/난도·핵심 문항·문항표·다음 학습을 읽는 단일 분석 글 |
| `/landing/matchup-board?manage=1` | 공개 자료실 위에 열리는 원장/관리자 게시물 관리 팝업 |
| `/landing/matchup-board?manage=1&compose=upload` | 관리 팝업과 PDF 직접 업로드 창을 함께 여는 진입점 |
| `/landing/reports`, `/landing/reports/:id`, `/landing/share/:token` | 기존 자동 생성 보고서 목록·상세·공유 호환 경로 |
| `/landing/board`, `/landing/board/:postId` | 외부 공개 자유게시판 목록·상세 |
| `/landing/board/write`, `/landing/board/:postId/edit` | 로그인 후 공개 게시글 작성·수정 |
| `/landing/reviews`, `/landing/reviews/:reviewId`, `/landing/reviews/write` | 공개 수강 후기 목록·상세·작성 |
| `/landing/scores`, `/landing/scores/:id` | 공개 시험 결과 통계 목록·상세 |
| `/landing/community/:boardType` | 학원 가족 전용 자유·질문·공지·자료 게시판 |
| `/landing/community/:boardType/posts/:postId`, `/landing/community/:boardType/write` | 학원 가족 전용 글 상세·작성 |

홈 내비게이션과 푸터에는 매치업 자료실을 항상 노출한다. 자동 생성 보고서가
설정되어 있어도 자료실 링크를 숨기지 않는다.

홈의 `시험 분석 노트`는 최신 공개본 2건을 종이 리포트 형태로 보여 준다. 홍보
문구를 크게 외치기보다 실제 분석 구조와 다음 수업 처방을 근거로 보여 주며,
빠른 메뉴와 매치업 햄버거 메뉴에서도 `/landing/analysis`로 진입할 수 있다.

정의되지 않은 `/landing/*` 주소는 인증 화면이나 다른 제품 루트로 보내지 않고
공개 홈 `/landing`으로 복구한다. 잘못된 커뮤니티 게시판 이름은 학원 가족
자유게시판으로 복구하며, 로그인이 필요한 작성·수정·커뮤니티 본문은 로그인 안내나
보호 상태를 명확히 표시한다.

## 게시와 공유 계약

1. 박철T가 매치업 파일을 컴퓨터에 저장하고 필요한 문구·출처를 직접 수정한다.
2. 홈페이지의 `PDF 자료 올리기`에서 20MB 이하 PDF를 선택한다.
3. 제목, 코멘트, 공개 시작·종료를 필요할 때만 입력하고 게시한다. 제목을
   비우면 파일명을 사용한다.
4. 관리 목록의 `링크 복사`로 단일 게시물 URL을 복사해 홍보 메시지에 넣는다.
5. 게시 후 원본 PDF는 게시 시점 스냅샷으로 보존한다. 컴퓨터 파일이나 자동
   보고서가 나중에 바뀌어도 이미 게시한 자료는 바뀌지 않는다.

기존 매치업 적중 보고서 편집기와 목록의 `홈페이지에 게시`도 같은
`PublicMatchupShowcase` 스냅샷을 생성한다. 보고서의 `submitted` 상태는 작성 완료와
편집 잠금만 뜻하며 공개 여부의 근거로 사용하지 않는다. 실제 공개 여부는 공개
자료실 목록 응답으로 확인한다. 작성 완료를 다시 열어도 이미 공개된 스냅샷은
유지되며, 공개 중단은 게시물 관리에서 별도로 실행한다.
`다음부터 묻지 않고 바로 게시`, 업로드 분할 방식, 자동 재분석 동의는 현재
tenant와 로그인 사용자 범위에서만 기억한다. 다른 학원이나 같은 PC의 다른
운영자에게 이어지지 않으며 저장소를 읽지 못하면 다시 확인하는 안전한 기본값을
사용한다.
자동 보고서를 게시할 때 공개 제목은 오래된 보고서 임시 제목보다 실제 시험
문서의 `document_title`을 우선해 중간·기말고사 이름이 뒤섞이지 않게 한다.
적중 보고서 목록은 공개 자료실 상태 조회가 끝나기 전에는 게시 버튼을 잠근다.
조회에 실패하면 빈 공개 목록으로 간주하지 않고 오류와 `자료실 상태 다시 확인`을
표시하며, 재시도가 성공한 뒤에만 게시를 허용한다. 이미 공개 또는 예약 공개 중인
보고서의 재시도는 백엔드가 기존 snapshot을 반환하므로 새 공개 행이나 R2 객체를
만들지 않는다.

문제 리뷰 도구의 `홈페이지 공개`는 선생님이 저장한 검수 버전만
`PublicProblemReviewShowcase`에 복제한다. 공개 본문과 PDF에는 원문 OCR 발췌,
내부 검토 메모, 유효성 메모, 신뢰도, 경고를 넣지 않는다. 재공개는 같은 게시물
URL을 유지하며 새 스냅샷이 안전하게 저장된 뒤 이전 PDF를 정리한다. 공개 중단은
도구 publication DELETE 계약으로 hidden 처리하고 스냅샷 데이터는 보존한다.

원장/관리자만 게시·수정·비공개·내리기를 실행할 수 있다. 게시물 관리는 별도
흰색 관리 페이지로 이동하지 않고 공개 자료실 위의 큰 팝업에서 처리한다. 직접
URL로 들어오는 기존 `/landing/admin/matchup-board` 경로는 호환용으로 유지한다.

공개 자료실은 원장으로 로그인한 상태에서도 학생·학부모와 같은 공개 응답만
사용한다. 초안·비공개·기간 밖 자료는 관리 팝업에서만 조회한다. 자료 내리기는
공개 목록에서 숨기되 사용자 PDF와 작성 내용은 서버 보존 정책을 따른다. 백엔드 소유 계약은
`backend/docs/domain/matchup.md`의 공개 자료실 절을 따른다.

2026-08-05 전환 시 기존 공개 자동 보고서 3건(중대부고·숙명여고·단대부고)은
원본을 변경하지 않고 새 공개 자료실용 정적 스냅샷으로 복구한다. 운영 검증용
제목과 설명을 가진 합성 게시물 5건은 정확한 ID와 R2 key를 대조한 뒤 제거하며,
실제 보고서와 사용자가 올린 PDF를 제목 패턴만으로 일괄 삭제하지 않는다.

## 표시와 실패 처리

- 공개 게시판·시험 통계·매치업·시험 분석 상세 본문은 GET으로만 읽는다. 상세가
  정상 표시된 뒤 별도 `POST .../{id}/view/`로 조회수를 기록하고 서버가 반환한
  최신 카운트를 화면 데이터에 합친다. 이 비핵심 기록이 실패해도 공개 본문을
  실패 화면으로 바꾸지 않는다. 새로고침·프리패치·크롤러 GET 자체는 조회수나
  다른 DB 상태를 변경하지 않는다.

- 홈과 목록은 PDF 렌더러를 미리 띄우지 않는다. 각 카드에 캐시된 대표 비교
  화면, 학교·시험 제목, 적중률·적중 문항 수를 먼저 보여준다. 카드 전체가 상세
  링크이며 작은 화살표만 별도 조작 대상으로 두지 않는다.
- 업로드·보존 원본은 PDF지만 공개 상세는 브라우저 내장 PDF 뷰어를 사용하지
  않는다. PDF.js가 각 쪽을 실제 본문 캔버스로 만들고 문서 순서대로 세로로 쌓는다.
  방문자는 별도 버튼이나 새 창 없이 첫 쪽부터 끝까지 홈페이지 자체를 스크롤한다.
  아직 화면에 닿지 않은 쪽은 가까워질 때 미리 렌더해 첫 표시 속도와 메모리 사용을
  함께 지킨다.
- 이 연속 본문 계약은 새 `/landing/matchup-board/:id`뿐 아니라 기존 학교별 적중
  사례 `/landing/reports/:id`와 유효한 카카오톡 공유 링크 `/landing/share/:token`에도
  동일하게 적용한다. 같은 자료가 진입 경로에 따라 대표 한 장과 새 창 버튼으로
  되돌아가지 않게 한다. 적중 자료가 0건인 공유 링크만 기존 빈 자료 안내를 유지한다.
- 모바일은 각 쪽을 가용 너비에 맞춰 축소하고 페이지 자체에 가로 overflow를 만들지
  않는다. 고밀도 모바일 화면은 기기 배율에 가까운 고해상도 캔버스로 렌더하고,
  화면에서 충분히 멀어진 쪽의 비트맵은 해제했다가 다시 접근할 때 재렌더해 긴 문서의
  메모리 누적을 막는다. `원본 PDF 다운로드`는 저장이 필요한 방문자를 위한 보조
  기능일 뿐, 읽기 위한 필수 동선이 아니다. 정적 대표 이미지는 홈과 목록 카드에만
  사용한다.
- 공개 본문은 iframe을 사용하지 않으므로 브라우저 PDF plugin이나 `frame-src`
  허용 여부에 의존하지 않는다. 원본 API 응답은 PDF.js가 직접 읽고 canvas로 표시한다.
- 시험 분석 상세는 PDF 뷰어를 본문으로 사용하지 않는다. 카페 게시물처럼 총평,
  출제 축, 단원·난도, 핵심 문항, 문항별 분석표, 다음 수업을 HTML로 이어서 읽고,
  공개 PDF는 저장·인쇄가 필요한 방문자용 보조 링크로만 제공한다. 긴 문항표는
  모바일에서 표 내부만 가로 스크롤하고 페이지 자체에는 가로 overflow를 만들지 않는다.
- 목록·상세 API가 실패하면 빈 화면으로 오인시키지 않고 오류와 다시 시도를
  표시한다. 자료가 없으면 첫 게시를 안내한다.
- 공개 홈 설정 요청은 공용 GET 재시도까지 실패해도 게시 중단으로 오인해 로그인
  화면으로 보내지 않는다. `/landing`에 연결 오류와 `다시 불러오기`를 표시하고,
  사용자가 재시도해 정상 응답을 받으면 같은 주소에서 공식 홈을 복구한다. 실제
  `has_landing=false` 정상 응답만 게시되지 않은 상태로 취급한다.
- 비공개·기간 만료·없는 게시물은 각각 접근 불가 상태를 안내하며 다른 테넌트
  자료로 대체하지 않는다.
- 공개 랜딩 설정은 테넌트별로 메모리 캐시하고 동시에 발생한 요청은 하나로
  합친다. 홈에서 자료 카드나 메뉴로 이동할 때 같은 설정을 화면마다 다시 받아
  로딩 상태에 머무르지 않게 한다. 같은 앱에서 관리자가 초안을 저장하거나 게시·
  게시 중단하면 해당 테넌트 캐시는 즉시 비운다.
- 대표 썸네일이 일시적으로 실패하면 깨진 이미지 아이콘 대신 `매치업 자료`
  대체 화면을 표시하고, 다른 이미지 URL로 바뀌면 다시 정상 로드를 시도한다.
- 연도 없는 `M/D 개강` 문구가 현재 연도 기준 이미 지난 날짜라면 원본 설정은
  보존하면서 공개 화면에서만 `개강 일정 문의`로 바꾸고 지난 날짜는 설명에서
  제거한다. 날짜 제거 전후의 마침표·쉼표도 자연스러운 한 문장으로 정리한다. 새
  일정은 관리자가 원본 설정에서 확정한다.

## 시각·반응형 계약

공식 홈은 딥 네이비와 선명한 블루를 유지하되 실제 수업 현장 사진을 핵심 근거로
사용한다. 인스타그램 화면이나 홍보 포스터 전체를 그대로 박지 않고, 현장 전경과
강사 인물을 분리해 구성한다. 영문 장식 문구 대신 자연스러운 한국어로 수업 준비,
현장 설명, 시험 후 매치업 결과가 이어지도록 쓴다. 과도한 골드 장식, 자동 회전
캐러셀, 긴 풀스크린 섹션 반복은 사용하지 않는다.

수강 문의는 두각학원을 첫 번째이자 주요 문의처로 표시하고, 명인학원과 박철 과학
연구소를 그 다음에 둔다. 홈의 주요 상담 버튼도 두각학원으로 연결한다. 공개 매치업
적중률은 백엔드 PDF 표지와 동일한 값으로 표시하며, 89.5%처럼 필요한 소수 첫째
자리를 반올림해 보존하고 일괄 정수 반올림으로 과장하지 않는다.

첫 화면의 강사 인물은 홈페이지 전용 정장 프로필 원본을 사용하고, 편집기에서
올린 수업 홍보 포스터를 강사 사진으로 대신 쓰지 않는다. 연두색 니트 프로필은
강사 소개 카드의 보조 컷으로만 사용해 공식적인 첫인상과 친근한 소개를 구분한다.
두 프로필은 원본 비율을 보존한 WebP로 제공하고 화면 크기에 따라 `object-fit`으로
잘라 쓰며, 실제 수업 현장 사진은 별도의 근거 이미지로 계속 노출한다.

공개 페이지의 작은 분류 문구와 상태 배지는 번역체 영문 장식 대신 자연스러운
한국어를 사용한다. 모바일의 게시판 카테고리와 후기 별점 필터는 숨은 가로 스크롤에
기대지 않고 줄바꿈해 마지막 항목까지 바로 누를 수 있어야 한다.

- 데스크톱은 1100~1366px에서 첫 화면 안에 정체성, 핵심 설명, 주요 CTA와 다음
  섹션의 시작이 함께 보여야 한다.
- 모바일 기준은 390px이며 320px까지 가로 페이지 overflow가 없어야 한다.
- 모바일 첫 화면은 전체 높이를 독점하지 않고 자료실로 바로 내려갈 수 있어야
  한다. 히어로는 고정 최소 높이를 두지 않고 실제 문구와 버튼 높이에 맞춰 줄어들며,
  핵심 정보와 바로 다음 주요 메뉴 사이에 64px를 넘는 빈 공간을 만들지 않는다.
  터치 대상은 서로 겹치지 않으며 내비게이션은 별도 메뉴로 접힌다.
- `prefers-reduced-motion`에서는 진입 애니메이션과 장식 이동을 끈다.

## 검증

```powershell
pnpm exec eslint src/landing/templates/PremiumDark.tsx src/landing/pages/LandingMatchupBoardPage.tsx src/landing/pages/LandingMatchupBoardDetailPage.tsx
pnpm exec playwright test e2e/landing-matchup-static-preview.spec.ts --project=chromium --reporter=list
pnpm exec playwright test e2e/landing-problem-analysis.mock.spec.ts --project=chromium --reporter=list
pnpm exec playwright test e2e/tchul-public-visitor-audit.spec.ts --project=chromium --reporter=list
pnpm build
```

`tchul-public-visitor-audit.spec.ts`는 운영 공개 API를 읽기만 하며 그 시점의
모든 매치업·시험 분석 상세 ID와 설정된 자동 보고서 ID를 동적으로 감사 목록에 넣는다.
위 표의 방문자 경로와 비회원 보호·없는 자료·잘못된 주소 상태를 데스크톱
1366x900과 모바일 390x844로 끝까지 스크롤하고, 본문·가로 overflow·깨진 이미지·
화면 밖 조작 요소를 누적 검사한 뒤 전체 스크린샷을 남긴다. PDF 뷰어·로그인
리다이렉트의 저장소·서비스워커 상태가 다음 주소를 오염하지 않도록 경로마다 새
브라우저 컨텍스트에서 검사하며, 한 경로 결함 때문에 뒤 경로 검수가 생략되지
않아야 한다. 모든 공개 매치업 상세는 iframe 부재, API 원본 응답, 실제 쪽 수와
본문 쪽 수 일치, 매 쪽 canvas 픽셀, 모바일 표시 너비를 끝까지 확인한다. iframe의
주소와 크기만으로 전체 PDF가 보인다고 판정하지 않는다. 느린 네트워크에서는 연속
본문의 쪽 수가 준비되거나 명시적인 오류 상태가 될 때까지 기다린 뒤 판정해 일시적인
로딩 화면을 본문 누락으로 오인하지 않는다.

시각 검수는 비로그인 공개 화면에서 위 전체 경로와 현재 공개된 모든 매치업·보고서
상세를 데스크톱 1366x900과 모바일 390x844로 직접 확인한다. 원장 화면은
`PDF 자료 올리기`가 자료실 위의 관리 팝업과 직접 업로드 탭으로 시작하는지
확인한다. 공개 목록 요청은 로그인 상태와 무관하게 공개 응답이어야 한다. 운영
자료를 생성한 검수라면 같은 실행에서 정확한 게시물 ID와 스토리지 키를 정리하고
익명 공개 목록에서 합성 제목이 0건인지 다시 읽는다.
