...
- 학생 홈의 제출 할 일은 과제 `submission_state=needs_submission`만 표시한다.
  `awaiting_review`는 성적 FAIL 이력이 남아도 제출을 완료한 상태이며,
  `reviewed`는 통과·보완 완료 상태다. 시험은 `submission_pending=true`인
  재제출 처리 중 이전 FAIL을 할 일에 다시 표시하지 않는다. 이 필드는
  성적·클리닉 판정을 바꾸지 않는다.
...
- 시험 등수와 상위 비율은 1차 점수 기준의 표준 공동 순위를 사용한다. 동점자는 같은
  ...
  계산한 값을 다시 매기지 않는다.
- 시험 카드·회차별 추이·평균·학생 상세의 기본 점수도 1차 원점수만 표시한다. 같은
  원시험의 2차 이상 재시험과 교사 PASS는 별도 보충 이력/완료 상태이며 원점수 그래프를
  덮어쓰지 않는다. 연결 정보 없이 별도 시험으로 저장된 과거 재시험은 기존 독립 시험으로
  계속 표시한다.
