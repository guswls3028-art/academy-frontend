...
The teacher submits up to five images and one current request. Each image keeps an independent student identity; “이 친구도” may inherit only the previous operation intent. The review receipt shows existing/new evidence, profile-link changes, lecture/session, `Attendance=ONLINE`, Alimtalk targets, and correction impact before confirm.
