...
프런트엔드 품질 작업은 패키지 관리자 준비와 필수 검사를 포함해 최대 15분을
사용한다. registry 준비가 약 7분 걸릴 때도 보안 감사·타입·lint·계약·빌드를
마칠 시간을 확보하기 위한 작업 예산이며, 각 검사의 실패는 계속 배포를 차단한다.
보안 감사 요청은 응답을 최대 120초 기다린다. 기본 60초에서 반복되던 registry
응답 중단을 줄이되, 기존 재시도 횟수와 취약점·연결 실패의 차단 동작은 유지한다.
