...
자동 PPT의 문항이 누락되거나 잘못 잘린 경우 같은 PDF에서 `직접 자르기`로
전환한다. 페이지를 넘기며 영역을 드래그해 여러 개 추가하거나 페이지 전체를
추가한 뒤 좌표를 숫자로 수정할 수 있다. 영역은 삭제·순서 변경이 가능하고,
선택 순서대로 기존 이미지 생성 작업에 제출된다. 수동 선택은 브라우저 메모리에만
있으며 새 PDF를 선택하면 지운다. 원본은 자동 재시도를 위해 화면에 남는다.
서버가 작업을 접수하면 브라우저에는 작업 ID·학원·사용자 ID·표시명·생성 시각만
최대 24시간 저장한다. 원본 파일, 직접 자른 좌표, 결과 다운로드 URL은 저장하지
않는다. 같은 브라우저에서 새로고침한 경우 현재 로그인 사용자·학원이 일치할
때만 `GET /api/v1/jobs/<job_id>/`로 기존 작업을 조회하며 재제출하지 않는다.
진행 중이면 계속 조회하고, 완료면 `이전 PPT 작업`에서 다운로드할 수 있다.
다운로드 버튼은 매번 작업을 다시 조회해 새 URL을 받는다. 다른 계정·학원,
로그아웃, 권한 거부, 만료·없는 작업에는 이전 결과를 보여 주지 않고 원본을
다시 선택할 수 있게 안내한다. 브라우저 저장소가 막혀도 현재 화면의 작업은
계속 진행하고 새로고침 복구가 제한됨을 알린다.
