...
관리자·교사는 타이머와 스톱워치 화면에서 현재 호스트의 테넌트 로고를
워터마크로 본다. 이동휘원소(`movementhui`)는 원본 PNG에 남색 사각 배경이
합성되어 있어 이 화면에서만 배경 없는 벡터 로고를 사용한다. 밝은 기본 화면에는
남색 글자·짙은 금색 궤도, 검은 프로젝터 화면에는 원본 브랜드의 크림색 글자·
노란 궤도를 사용한다. 모드·전체 화면을 전환해도 현재 표시 상태에 맞는 로고가
이어지고 타이머 값과 동작은 영향을 받지 않는다. 로고 파일이 로드되지 않아도
타이머·스톱워치는 계속 동작하며, 다른 테넌트와 로그인·헤더의 원본 로고는
기존 경로를 사용한다. 테넌트 판별은 기존 호스트 기반 해석과 권한 경계를 따른다.
기본·프로젝터·전체 화면에서 로고 배경, 가독성, 390px 화면을 확인한다.
좁은 화면에서는 학원명과 모드·프로젝터 조작을 두 줄로 배치해 버튼 글자가
세로로 압축되지 않도록 한다. 시간 프리셋·직접 입력·시계 숫자는 390px 폭에
맞춰 배치하며 화면 높이가 부족하면 타이머 내부를 스크롤해 조작할 수 있다.
